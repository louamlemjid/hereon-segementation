...
class CustomUNet(nn.Module):
    ...
    def visualize_architecture(self,input_size=(3, 256, 256), device=None):
        """
        Print a clean architecture summary using torchsummary.

        Args:
            input_size: tuple -> (channels, height, width)
        ...
        print(f"\n🔍 Model Summary on {device.upper()}\n")

        summary(model, input_size=input_size)


    def visualize_model(self, input_size=(1, 3, 256, 256), save_path="unet_graph"):
        """
        Visualize a PyTorch model using torchviz.

        Args:
            model: torch.nn.Module
            input_size: tuple -> (batch, channels, height, width)
            save_path: str -> file name (without extension)
        """
        self.eval()
